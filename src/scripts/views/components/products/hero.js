...
const hero = {
	async init(){
		...
		<div class="w-full h-[360px] bg-black flex flex-col relative hero-wrap md:h-[460px] lg:h-[500px]">
			<img src="./images/loading.gif" data-src="${this.dataProducts[0].img[0]}" class="lazyload w-full h-full absolute z-0 object-cover select-none image-hero"></img>
	...
	slideHero(element,i,time){
		let template = `<img src="./images/loading.gif" data-src="${this.dataProducts[i].img[0]}" class="lazyload w-full h-full absolute z-0 select-none image-hero object-cover"></img>`;
		setTimeout(()=>{
			i++;
			if(i >= this.maxImage){
				i=1;
